...
let result_encoding encoding =
  ...
  conv
    ...
    | Error (code, descr) -> (false, descr, code, None))
    (fun (ok, descr, code, result) ->
       if ok then Ok result else Error (code, descr))
    (obj4
       (req "ok" bool)
...
let getMe () =
  ...
  | Ok (Some user) -> user
  | Ok None -> invalid_arg "telegramAPI"
  | Error (code, msg) -> raise (TelegramError (code, msg))
